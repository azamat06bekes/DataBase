...
SELECT
    ...
    MAX(first_name || ' ' || last_name) FILTER (WHERE salary = (SELECT MAX(salary) FROM employees e2 WHERE e2.department = e.department)) AS highest_paid,
    GREATEST(MAX(salary), MIN(salary)) AS salary_check,
    LEAST(MAX(salary), MIN(salary)) AS minmax_check
FROM employees e
GROUP BY department;
